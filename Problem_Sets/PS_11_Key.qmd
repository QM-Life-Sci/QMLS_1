...
M <- read_csv("../data/Butterfly_Gene_Expression.csv", show_col_types = FALSE)
...
The major question you want to ask is: What is the relative variation between *measurement* vs. between *specimen*?
...
![](https://i.imgur.com/CVTtMkJ.png){fig-width=50%, fig-alt="Photograph of a walking stick."}

Because the measurements are small (~ 0.25 cm), there should be concern that they will not be very repeatable. Before collecting all the data, you would want to estimate the intraclass correlation coefficient of these measurements.

There are 2 measurements each on 25 different walking stick femurs, which are included in the files `WalkingStickFemurs.csv`.
...
Based on your plot, do you expect repeatability to be high or low? Give an estimate of what you think the repeatability is.

> The points are fairly close to the line, so repeatability should be ok. 0.6 to 0.8 is a reasonable guess.
...
> The numbers are very similar. This is probably just due to rounding error introduced by manually copying the values from the first model. Overall, the measurements are pretty repeatable.

Would you be comfortable going forward with data collection given this value for ICC?

> Maybe or maybe not. If we expect fairly large differences between groups, then it's probably fine. But if the differences are small, then there may be so much variation associated with taking measurements that it will swamp out the variation between groups.
