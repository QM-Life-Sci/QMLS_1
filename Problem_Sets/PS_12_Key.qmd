...
stalk |>
  ...
  summarize(st_dev = sd(eye_span), .groups = "drop")

# Some thoughtful plotting
ggplot(stalk, aes(x = food_source, y = eye_span)) +
  geom_point(
    position = position_jitter(width = 0.05, seed = 4588),
    alpha = 0.5
  ) +
...
t_rand <- function(.x, stalk) {
  # the 1st parameter is the iterator .x
...
Refer to the key for PS 08 to refresh yourself with the question and adapt the code from the example above (either sequential or parallel).

- You will have to use `lm()` to fit the ANOVA
- You will need to use the overall `F`-value for the model (instead of the `t`-statistic).

Otherwise we will leave this exercise undirected.
...
ggplot(HT, aes(x = Mismatch_Degree, y = Survival_Days)) +
  geom_point(
    position = position_jitter(width = 0.05, seed = 3648),
    alpha = 0.5
  ) +
...
for (ii in 2:nreps) {
  fm_rand <- lm(sample(Survival_Days) ~ Mismatch_Degree, data = HT)
...
ggplot(Sole, aes(x = PC1, y = as.numeric(Solea_solea) - 1)) +
