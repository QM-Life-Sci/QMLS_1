...
```{r}
#| fig-alt: "Histogram of height showing a normal distribution between 160 and 200 cm but with a wide range of heights."
...
> The histogram has a wide range of values from 80 to 160 cm and then what looks like a normal distribution.
...
> From age 0 to ~20, height increases. Then it becomes steady or decreases slightly. Now we can see that the small measures in the histogram are growing individuals.

Filter out any individuals less than Age 20. Make a second histogram of just this data.

```{r}
#| fig-alt: "Histogram of height for individuals over age 20 showing a normal distribution between 160 and 200 cm."
...
> The values range from about 160 to 200 and are peaked in the middle, around 175 cm.
...
```{r}
# FIXME
gr <- read_csv("../data/growthrate_data.csv", show_col_types = FALSE)
gr
```
...
> The csv has the header row separate. But the number of data rows matches.

- How many columns are in `gr`? Does this agree with the number of columns in the csv file? Why or why not?

> 13. This matches. No header issues.
...
# There are a few ways to pivot these data. All of which
# pass the "day_" columns into pivot_longer() as the "cols"
# parameter.
...
gr_long2 <- gr |>
  pivot_longer(cols = c(3:13), names_to = "day", values_to = "biomass")
gr_long2
...
# You can also remove this extraneous column by passing NA
# to the into = parameter to directly drop the new column
gr_long2 <- gr_long |>
  separate(day, into = c(NA, "days"), convert = TRUE)
gr_long2

# You can use select() with - to do drop the new "day" column
# in a pipe:
gr_long3 <- gr_long |>
  separate(day, into = c("text", "days"), convert = TRUE) |>
  select(-text)
...
# We will need gr_long later, so let's just overwrite it:
gr_long <- gr_long |>
  separate(day, into = c(NA, "days"), convert = TRUE)
...
> No, now we see we have different sample sizes per species (52, 53, 54, 55, 55).
