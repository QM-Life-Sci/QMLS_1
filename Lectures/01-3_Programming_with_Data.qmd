...
![](http://s3.crackedcdn.com/phpimages/article/0/2/5/208025_v1.jpg){fig-align="center" fig-alt="A painting depicting death and destruction in the Dark Ages."}
...
summary(tvtest)
ggplot(tvdat, aes(x=log(DD5_TOT),y=sqrt(tv),col=Region))+geom_point(shape = 1, size=3,alpha=1/3)+geom_smooth(aes(col=Region),method='lm')+xlab(bquote('log(Degree Days above'~5^o~C*')'))+ylab('sqrt(Terminal Velocity (m/s))')
```
...
![](https://i.imgur.com/1yKQBJZ.png){fig-align="center" fig-alt="Three rows of images showing a 3 by 3 matrix with the middle row, left column, and second row last element chosen."}
...
![](../images/Workflow.drawio.png){fig-align="center" width="80%" fig-alt="Flow chart showing the interrelationships between different tidyverse packages for data analysis."}
