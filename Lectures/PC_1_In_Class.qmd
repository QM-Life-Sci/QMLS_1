...
Again for rattlesnake data: it was asked to compare the raw csv to each tibble we made--when I tried to do this by slice_head() for the new tibble, it printed the whole tibble. I even did slice_head(n=2) for raw and new tibble to see if the second value in raw was the same as the first one in the new tibble, and it still printed the whole tibble for the new one I made without the NAs. Do you know what the issue might be?
...
I don't fully understand when to use variables and manipulations within the ggplot() function vs. within the geom() function. 
